...
export function BlogCard({ post, index }: BlogCardProps) {
  ...
  return (
    ...
    >
      <Link href={`/blog/${post.slug}`} className="block group h-full" aria-label={`Read full article: ${post.title}`}>
        <article className="flex flex-col h-full bg-card/50 backdrop-blur-sm rounded-xl overflow-hidden border border-border/50 transition-all duration-300 hover:shadow-xl hover:border-primary/20">
          ...
          <div className="p-6 flex-1 flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="relative w-10 h-10 rounded-full overflow-hidden border-2 border-primary/20">
                  <Image src={avatarSrc} alt={post.author.name} fill className="object-cover" />
                </div>
            ...
            <h2 className="text-xl font-semibold mb-3 text-foreground group-hover:text-primary transition-colors line-clamp-2 leading-tight">{post.title}</h2>

            <p className="text-muted-foreground mb-4 line-clamp-2 leading-relaxed flex-grow">{post.description}</p>
