...
const TARGETS = [
  ...
  {
    name: "CodeSenseiSearch (FE)",
    url: "https://code-sensei-search-web.vercel.app",
    kind: "site",
  },
  {
    name: "CodeSenseiSearch (API)",
    url: "https://code-sensei-search-api.vercel.app",
    kind: "api",
  },
...
function gradeSite(html, probes) {
  ...
  return {
    rows: [
      ...
      [
        "twitter:creator",
        r(h.twCreator === "@ShaileshWork"),
        h.twCreator || "(should be @ShaileshWork)",
      ],
      ...
      [
        "icon links",
        r((h.icons?.length || 0) > 0),
        `${h.icons?.length || 0} found`,
      ],
      ["favicon.ico", r(probes["favicon.ico"]?.ok), ""],
      [
        "apple-touch-icon",
        r(
          probes["apple-touch-icon.png"]?.ok ||
            probes["apple-icon.png"]?.ok ||
            probes["apple-icon-noext"]?.ok
        ),
        "",
      ],
...
async function auditOne(target) {
  ...
  if (!home.ok) {
    console.log(
      `  ✗ home unreachable (status ${home.status || "ERR"} ${home.error || ""})`
    );
...
(async () => {
  ...
  for (const r of results) {
    if (!r.reachable) {
      console.log(`  ✗ ${r.target.name}: UNREACHABLE`);
      continue;
    }
    const misses = r.graded.rows
      .filter((x) => x[1].trim() === "MISS")
      .map((x) => x[0]);
    const hits = r.graded.rows.length - misses.length;
    console.log(
      `  ${r.target.name}: ${hits}/${r.graded.rows.length} OK — missing: ${misses.join(", ") || "none"}`
    );
  }
})();
