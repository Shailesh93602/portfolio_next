...
- **Claims a recruiter checks by clicking (2026-09-06):** GfG = `PROFILE_META.gfgLine` (rendered from `PROFILE.achievements.problemsSolved`, a **floor with slack** — 640+, against a live profile showing 649) — **volume only, no rank**. The figure is **not monotonic**: it was 650 on 2026-09-06 and 649 on 2026-09-20, which turned the stated floor false and the daily claim check red. Never restate it as an exact count. "Institute Rank 1" was dropped: a rank is only as meaningful as the population it ranks within, and that profile's institute is **eSparkBiz Technologies** (a former employer), not a college, so the claim deflated on click. HackerRank = "5-star C++" only (Python is 3★, no Problem Solving badge); no CodeChef achievement card (1★). `scripts/check-project-claims.mjs` verifies the number daily from the profile's embedded JSON (`total_problems_solved`); the `institute_rank` row went with the claim, because a check that verifies a number no surface states goes stale unnoticed.
- **No Java / Spring Boot / Python contribution claim anywhere (2026-09-06).** He can modify and ship those services with AI help but has said he cannot answer interview questions on them, and the ATS gain is marginal for a TypeScript/Node target. What replaces it is the honest framing, not silence: the platform is **polyglot** and he **owns the Node.js/TypeScript side** — that phrasing is on the resume, `/about` (`constants/index.ts`) and both llms files, asserted by `__tests__/identity-claims.test.ts`, which bans "Spring Boot" and "AI-assisted development" across the scanned surfaces.
...
npm test                # Jest unit tests — paste the gate's number, never an estimate
...
content/
  blog/                 # 26 MDX files (17 archived, 9 published) — YAML frontmatter + **HTML** body. Not MDX: there is no MDX compiler, and `content/blog/` is in `.prettierignore` because Prettier parses .mdx as JSX and inserts `{" "}` tokens that ship as literal text. Add new posts here, then add slug to BLOG_SLUGS in blog-data.ts.
...
- **Unit tests** (`__tests__/`): see the gate's own number — paste it, never estimate it. Covers API routes (statistics, contact), blog functions, components (BlogCard, ProjectCard, EducationSection, KeyMetrics), utils, constants. Run with `npm test`.
- **E2E** (`e2e/`):
...
- Blog posts: add MDX to `content/blog/`, then add the slug to `BLOG_SLUGS` in `lib/blog-data.ts`. (`next-sitemap` and `data/blog-manifest.json` are gone as of 2026-09-20: the config was never wired to `postbuild`, nothing read the manifest it existed for, and had anyone wired it up the standard way it would have written `public/robots.txt` and `public/sitemap.xml` over the App Router routes — reintroducing `lastModified: new Date()` on every URL, named AI-crawler groups that repeat no disallow list, and `/hire` at priority 0.8 though it is a permanent redirect.) `archived: true` in frontmatter keeps a post at its URL (200) but out of the index / home / RSS / tags **and the sitemap**, served with `<meta name="robots" content="noindex, follow">` from `generateMetadata` — and deliberately NOT disallowed in robots.txt (Disallow + noindex cancel). The 17 posts from 2024 are archived this way; `publishedPosts` is what surfaces list, `blogPosts` is everything that renders. Guarded by `__tests__/archived-posts.test.ts` and the sitemap tests in `e2e/meta-and-schema.spec.ts`. There is no `/api/blogs` route any more — it served a stale hand-copied duplicate of the post list (with the "700+" title) that nothing consumed.
